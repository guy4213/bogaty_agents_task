...
interface CommentsGridProps { assets: AssetRecord[]; }

function extractComments(asset: AssetRecord): CommentItem[] {
  if (!asset.content) return [];
  const content = asset.content;
  if (Array.isArray(content)) return content as CommentItem[];
...
export function CommentsGrid({ assets }: CommentsGridProps) {
  const textAssets = assets.filter((a) => a.asset_type === 'text' || a.file_format === 'json');
  const allComments = textAssets.flatMap((asset) =>
    extractComments(asset).map((c) => ({ comment: c, validationPassed: asset.validation_passed }))
  );
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      {allComments.map(({ comment, validationPassed }, i) => (
        <CommentCard key={i} comment={comment} index={i} validationPassed={validationPassed} />
      ))}
    </div>
  );
}

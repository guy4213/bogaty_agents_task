...
function groupByItem(assets: AssetRecord[]): ItemGroup[] {
  ...
  for (const asset of assets) {
    ...
    if (!map.has(idx)) map.set(idx, { itemIndex: idx, imageAsset: undefined, captionAsset: undefined });
    const group = map.get(idx)!;
    if (asset.asset_type === 'image' || asset.file_format === 'png') {
      group.imageAsset = asset;
    } else if (asset.asset_type === 'text' || asset.asset_type === 'caption' || asset.file_format === 'json' || asset.file_format === 'txt') {
...
export function PostsGrid({ assets, aspect }: PostsGridProps) {
  ...
  if (groups.length === 0) {
    return <p className="py-8 text-sm text-center" style={{ color: 'var(--fg3)' }}>No posts to display.</p>;
  }
  return (
